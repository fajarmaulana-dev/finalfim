...
const getAllContestants = async (req, res, next) => {
  let contestant;
  try {
    contestant = await MCSHSContestant.find({});
  } catch (err) {
...
const getContestantById = async (req, res, next) => {
  ...
  try {
    contestant = await MCSHSContestant.findById(contestantId);
  } catch (err) {
...
const createContestant = async (req, res, next) => {
  ...
  const createdContestant = new MCSHSContestant({
    name,
    color,
    score: 100,
  });
...
const resetContestants = async (req, res, next) => {
  try {
    await MCSHSContestant.updateMany({}, [{ $set: { score: 100 } }]);
  } catch (err) {
    const error = new HttpError(
