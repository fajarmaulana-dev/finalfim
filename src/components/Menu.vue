...
<template>
    <header style="transition: .4s;"
        ...
        <div
            ...
            <router-link to="/auth" title="Perbarui Kata Sandi" :class="route.path == '/auth' ? 'hidden' : 'hidden sm:grid'"
                class="font-extrabold text-sky-600 hover:text-sky-700 active:text-sky-600 pr-2">
                Halo, {{ user?.name }} !
            </router-link>
            <button @click="router.push('/auth')" title="Perbarui Kata Sandi" class="fa-solid fa-user"
                :class="route.path == '/auth' ? 'hidden' : 'grid sm:hidden'"></button>
            ...
            <button @click="router.push(`${route.path}/edit`)" title="Edit Soal" class="fa-solid fa-pen"
                :class="route.path.includes('edit') ? 'hidden' : (route.path == '/auth' ? 'hidden' : 'grid')"></button>
            <button @click="router.push(`/${route.path.split('/')[1]}`)" title="Kembali ke Kuis"
    ...
    <footer
        ...
        <router-link v-for="foot, i in footer" :key="i" :to="route.path.includes('edit') ? `/${foot}/edit` : `/${foot}`"
            class="w-full h-full grid place-items-center" :class="route.path.includes(foot) ? 'bg-sky-200' : ''">
